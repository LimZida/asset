...
function getItem(label, key, icon, children, type) {
  ...
  const items = [
    getItem(<Link to="">메인화면</Link>, '1', <LayoutOutlined />),
    getItem('MENU', 'sub1', <UserOutlined />, [
      getItem(<Link to="hardwares">하드웨어</Link>, '2', <HddOutlined />),
      getItem(<Link to="softwares">소프트웨어</Link>, '3', <LaptopOutlined />),
    ],'group'),
    getItem('SETTING', 'sub2', <UserOutlined />, [
        getItem(<Link to="codes">코드관리</Link>, '4', <HddOutlined />),
        getItem(<Link to="employee">직원관리</Link>, '5', <SettingOutlined />),
        getItem(<Link to="history">히스토리</Link>, '6', <HistoryOutlined />),
      ],'group'),
  ];
...
const BreadcrumbMenu = function(selectedMenu){
  ...
  return(
    <>
    {
      ...
      >
        <Breadcrumb.Item>{(sKey === '2' ||  sKey === '3') ? sub1Arr[0].label : sub2Arr[0].label}
        </Breadcrumb.Item>
        <Breadcrumb.Item>{ sName }
        </Breadcrumb.Item>
      </Breadcrumb>
    }
    </>
